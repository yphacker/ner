...
class HMM(object):
    ...
    def train(self, word_lists, tag_lists):
        ...
        参数:
            word_lists: 列表，其中每个元素由字组成的列表，如 ['担','任','科','员']，对应的id
            tag_lists: 列表，其中每个元素是由对应的标注组成的列表，如 ['O','O','B-TITLE', 'E-TITLE']对应的id
        """
        ...
        for tag_list in tag_lists:
            ...
            for i in range(seq_len - 1):
                current_tagid = tag_list[i]
                next_tagid = tag_list[i + 1]
        ...
        for tag_list, word_list in zip(tag_lists, word_lists):
            ...
            for tag_id, word_id in zip(tag_list, word_list):
    ...
    def predict(self, word_lists):
        pred_tag_lists = []
        for word_list in tqdm(word_lists):
    ...
    def decoding(self, word_list):
        ...
        for step in range(1, seq_len):
            ...
            bt = Bt[wordid]
            for tag_id in range(len(config.label2id)):
        ...
        tag_list = [_id for _id in reversed(best_path)]
        return tag_list
