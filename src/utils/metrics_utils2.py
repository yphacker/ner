...
def get_score(y_true_list, y_pred_list):
    ...
    for i, label in enumerate(y_true_list):
        print(y_true_list[i], len(y_true_list[i]))
        print(y_pred_list[i], len(y_pred_list[i]))
        temp_1 = []
        temp_2 = []
        for j, m in enumerate(label):
            if j == 0:
                continue
            elif y_true_list[i][j] == config.label2id['[SEP]']:
                metric.update(pred_paths=[temp_2], label_paths=[temp_1])
                break
            else:
                temp_1.append(config.id2label[y_true_list[i][j]])
                temp_2.append(y_pred_list[i][j])
